...
if (property_exists($model_class_name,
        'crud_fields_list_arr') && (count($model_class_name::$crud_fields_list_arr) > 0)) {
...
<div>
                            ...
                            <form class="form-inline">
                                ...
                                foreach ($model_class_name::$crud_model_filtered_field_arr_for_list as $field_name) {
                                    ...
                                    <div class="form-group">
                                        <label><?php echo CRUDUtils::getTitleForField($model_class_name,
                                                $field_name) ?></label>
                                        ...
                                        echo Widgets::renderFieldWithWidget($field_name, $obj, $context_field_value)
                                        ?>
                                    </div>
                                    <?php
                                }
    ...
    <?php
    if (CRUDUtils::canDisplayCreateButton($model_class_name, $context_arr)) {
        ...
        <table class="table table-striped table-hover">
            ...
            if ($crud_fields_list_arr) {
                ...
                <colgroup>
                    ...
                    foreach ($crud_fields_list_arr as $field_arr) {
                        ?>
                        <col<?php echo(array_key_exists('col_class',
                            $field_arr) ? ' class="' . $field_arr['col_class'] . '"' : ''); ?>>
                        <?php
                    }
                    ?>
                </colgroup>
            ...
            <tr>
                ...
                foreach ($props_arr as $prop_obj) {
                    $table_title = CRUDUtils::getTitleForField($model_class_name, $prop_obj->getName());
            ...
            foreach ($objs_ids_arr as $obj_id) {
                $obj_obj = CRUDUtils::createAndLoadObject($model_class_name, $obj_id);
                ?>
                <tr>
                    <?php
                    foreach ($props_arr as $prop_obj) {
                        $link_field_key = array_search($prop_obj->getName(), array_values($container_models_arr));
                        $roles = Widgets::getFieldWidgetName($prop_obj->getName(), $obj_obj);
                        ...
                        if ($link_field_key !== false) {
                            ...
                            $container_obj = CRUDUtils::createAndLoadObject($container_model,
                                $prop_obj->getValue($obj_obj));
                            ...
                            if ($roles == 'options') {
                                $role = Widgets::getFieldWidgetOptionsArr($prop_obj->getName(), $obj_obj);
                                if (array_key_exists($prop_obj->getValue($obj_obj), $role)) {
                                    $title = $role[$prop_obj->getValue($obj_obj)];
                                }
                            } else {
                                $title = Widgets::renderListFieldWithWidget($prop_obj->getName(), $obj_obj);
                                ...
                                if (property_exists($model_class_name, 'crud_model_title_field')) {
                                    if ($prop_obj->getName() == $model_class_name::$crud_model_title_field) {
                                        if (CRUDUtils::stringCanBeUsedAsLinkText($title)) {
